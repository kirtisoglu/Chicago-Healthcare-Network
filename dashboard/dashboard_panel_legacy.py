...
from falcomplot.mapping import build_basemap, add_hierarchy, add_markers, add_choropleth
from healthcare.health_atlas import load_health_atlas_data, INDICATOR_LABELS
...
# Palette options for the choropleth colour selector
PALETTES = ["YlOrRd", "Blues", "Greens", "RdYlGn", "PuRd", "BuPu", "Oranges"]
...
# Each layer entry: {"name": str, "type": "boundary"|"hierarchy"|"facilities"|"choropleth",
#                    "visible": bool, ...rebuild kwargs}
_current = {"city": None, "layers": []}

# Lazy-loaded health atlas GeoDataFrame (loaded once on first use)
_health_atlas_gdf: gpd.GeoDataFrame | None = None


def _get_health_atlas_gdf() -> gpd.GeoDataFrame | None:
    """Return the cached health atlas GeoDataFrame, loading it on first call."""
    global _health_atlas_gdf
    if _health_atlas_gdf is None:
        try:
            _health_atlas_gdf = load_health_atlas_data(geo=True)
        except Exception as exc:
            status.object = f"**Health Atlas load error:** {exc}"
    return _health_atlas_gdf
...
def _rebuild_map():
    ...
    for layer in _current["layers"]:
        ...
        if layer["type"] == "hierarchy":
            ...
            tooltip_extra = ["community_name"] if "community_name" in gdf.columns else []
            add_choropleth(
                m, gdf, col,
                label=lbl,
                palette=palette,
                tooltip_fields=tooltip_extra,
            )
            ...
            if gdf.empty:
                continue
            # Build a categories dict covering every actual category in the gdf,
            # falling back to the layer colour/radius for unrecognised types.
            from falcomplot.mapping import HEALTH_CATEGORIES as _HC
            all_cats = gdf["category"].dropna().unique()
            cats = {}
            for order, ct in enumerate(sorted(all_cats)):
                cats[ct] = _HC.get(ct, {
                    "color": layer["color"],
                    "radius": layer["radius"],
                    "order": order,
...
def _rebuild_filter_panel():
    ...
    Boundary / hierarchy / choropleth: one visibility toggle (no count).
    ...
    for i, layer in enumerate(_current["layers"]):
        ...
        if ltype in ("boundary", "hierarchy", "choropleth"):
            t = pn.widgets.Toggle(
                name=layer["name"],
                value=layer["visible"],
                width=230,
...
# ──────────────────────────────────────────────────────────────────────────────
# SECTION 5 — HEALTH ZONES (Chicago Health Atlas choropleth)
# ──────────────────────────────────────────────────────────────────────────────

# Build indicator options list: "Human Label" → col_name
_indicator_options = {"— Select Indicator —": None}
_indicator_options.update({lbl: col for col, lbl in sorted(INDICATOR_LABELS.items(), key=lambda x: x[1])})

health_indicator_select = pn.widgets.Select(
    name="Indicator",
    options=list(_indicator_options.keys()),
    value="— Select Indicator —",
    width=230,
)
health_palette_select = pn.widgets.Select(
    name="Colour Palette",
    options=PALETTES,
    value="YlOrRd",
    width=230,
)
health_load_btn = pn.widgets.Button(
    name="Load Health Atlas Data",
    button_type="primary",
    width=230,
)
health_add_btn = pn.widgets.Button(
    name="Add Choropleth Layer",
    button_type="success",
    width=230,
    disabled=True,
)
health_status = pn.pane.Markdown("*Data not loaded yet.*", width=230)

health_arrow, health_body = _make_section(
    "Health Zones",
    [health_status, health_load_btn, health_indicator_select, health_palette_select, health_add_btn],
    expanded=False,
)


def _on_load_health_atlas(*_):
    health_status.object = "Loading Chicago Health Atlas data…"
    health_add_btn.disabled = True
    gdf = _get_health_atlas_gdf()
    if gdf is not None:
        n = len(gdf)
        health_status.object = f"✓ {n} community areas loaded."
        health_add_btn.disabled = False
    else:
        health_status.object = "**Failed to load health atlas data.**"


def _on_add_choropleth(event):
    if not _current.get("city"):
        status.object = "**Load a boundary first.**"
        return

    col = _indicator_options.get(health_indicator_select.value)
    if col is None:
        status.object = "**Select an indicator first.**"
        return

    gdf = _get_health_atlas_gdf()
    if gdf is None:
        status.object = "**Health Atlas data not loaded.**"
        return

    lbl = INDICATOR_LABELS.get(col, col)
    palette = health_palette_select.value
    layer_name = f"Health Zone — {lbl}"

    # Replace any existing choropleth layer with same name
    ...
    status.object = f"Adding **{lbl}** choropleth…"
    _rebuild_map()
    _rebuild_filter_panel()
    filter_arrow.value = True
    status.object = f"✓ **{lbl}** choropleth added."


health_load_btn.on_click(_on_load_health_atlas)
health_add_btn.on_click(_on_add_choropleth)
...
def _on_load(event):
    ...
    filter_arrow.value = True
    health_arrow.value = True      # expand Health Zones
